...
internal static class AnalyticsErrorMapper
{
    ...
    /// </summary>
    /// <param name="queryError">The error from the server response.</param>
    /// <returns>An AnalyticsException or one of its subclasses.</returns>
    private static AnalyticsException MapServerErrorCode(QueryError queryError, ErrorContext errorContext)
    {
        return queryError.Code switch
        {
            20000 => new InvalidCredentialException(queryError.Message, errorContext),
            21002 => new AnalyticsTimeoutException($"{queryError.Message}. Error code: {queryError.Code}", errorContext),
            _ => new QueryException(queryError.Message, errorContext) { Code = queryError.Code, ServerMessage = queryError.Message }
        };
    }
    ...
    /// <returns>An AnalyticsException or one of its subclasses.</returns>
    internal static AnalyticsException MapServiceErrors(IReadOnlyList<QueryError> errors, ErrorContext errorContext)
    {
    ...
    internal static bool AreErrorsRetriable(IReadOnlyList<QueryError> errors)
    {
        if (errors == null || errors.Count == 0)
        {
            return false;
